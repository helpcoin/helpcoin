...
// Copyright (c) 2013-2014 Helpcoin Developers
...
uint256 hashGenesisBlock("0x21a0a13ed359635356176c374d61543149783be1483329260c62d39ceb041e87");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Helpcoin: starting difficulty is 1 / 2^12
...
{
    // Helpcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 5000 * COIN;       // 5000 coins per one generated block
...
static const int64 nTargetTimespan = 24 * 60 * 60; // readjust diffuculty every day
static const int64 nTargetSpacing = 5 * 60; // HelpCoin: every 5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Helpcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
{
    // Helpcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "HelpCoin creation generation";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 88 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0x040184710fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1391807362;
        ...
        assert(block.hashMerkleRoot == uint256("0x3760183707abc3c2e87776ac8958ab6fae7074a858c3b5a9471ffb606122dc01"));  ///////////////////////////////////////////
        block.print();
        //assert(hash == hashGenesisBlock);
        if (true && block.GetHash() != hashGenesisBlock)
        //nachalo poiska Genesis Hash
        {
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("HelpcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("HelpcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static HelpcoinMiner(CWallet *pwallet)
{
    printf("HelpcoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running HelpcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("HelpcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&HelpcoinMiner, pwallet));
}
